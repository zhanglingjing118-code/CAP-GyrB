...
"""
"""
def build_loader(cfg, logger): #ZLJ
    train_dataset, valid_dataset, test_dataset = build_dataset(cfg, logger)
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size_s, shuffle=True)
    valid_dataloader = DataLoader(valid_dataset, batch_size=batch_size_s)
    test_dataloader = DataLoader(test_dataset, batch_size=batch_size_s)
    weights = train_dataset.weights

    return train_dataloader, valid_dataloader, test_dataloader, weights
"""
"""
def build_loader_mtl(cfg, logger):#LiGang
    train_dataloader_list = []
    valid_dataloader_list = []
    test_dataloader_list = []
    weights_list = []

    dataset_mtl_list = ["a_input_plus_random", "b_input_plus_random"]#注意，样本量从左往右依次递减！
    tmp = cfg.DATA.DATASET
    # print('---')#
    # print(tmp)#
    # print('---')#
    for dataset_mtl in dataset_mtl_list:
        cfg.defrost()
        cfg.DATA.DATASET = dataset_mtl
        cfg.freeze()
        train_loader, val_loader, test_loader, weights = build_loader(cfg, logger)
        train_dataloader_list.append(train_loader)
        valid_dataloader_list.append(val_loader)
        test_dataloader_list.append(test_loader)
        weights_list.append(weights)
    cfg.defrost()
    cfg.DATA.DATASET = tmp
    cfg.freeze()
    
    return train_dataloader_list, valid_dataloader_list, test_dataloader_list, weights_list
"""
def build_loader_mtl(cfg, logger):#ZLJ
    train_dataloader_list = []
    valid_dataloader_list = []
    test_dataloader_list = []
    weights_list = []
    ...
    # print('---')#
    for dataset_mtl in dataset_mtl_list:
        if dataset_mtl=="a_input_plus_random":
            cfg.defrost()
            cfg.DATA.DATASET = dataset_mtl
            cfg.freeze()
            batch_size_a = cfg.DATA.BATCH_SIZE_a
            train_dataset, valid_dataset, test_dataset = build_dataset(cfg, logger)
            train_loader = DataLoader(train_dataset, batch_size=batch_size_a, shuffle=True)
            val_loader = DataLoader(valid_dataset, batch_size=batch_size_a)
            test_loader = DataLoader(test_dataset, batch_size=batch_size_a)
            weights = train_dataset.weights
            train_dataloader_list.append(train_loader)
            valid_dataloader_list.append(val_loader)
            test_dataloader_list.append(test_loader)
            weights_list.append(weights)
        if dataset_mtl=="b_input_plus_random":
            cfg.defrost()
            cfg.DATA.DATASET = dataset_mtl
            cfg.freeze()
            batch_size_b = cfg.DATA.BATCH_SIZE_b
            train_dataset, valid_dataset, test_dataset = build_dataset(cfg, logger)
            train_loader = DataLoader(train_dataset, batch_size=batch_size_b, shuffle=True)
            val_loader = DataLoader(valid_dataset, batch_size=batch_size_b)
            test_loader = DataLoader(test_dataset, batch_size=batch_size_b)
            weights = train_dataset.weights
            train_dataloader_list.append(train_loader)
            valid_dataloader_list.append(val_loader)
            test_dataloader_list.append(test_loader)
