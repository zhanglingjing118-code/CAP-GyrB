...
class HiGNN(torch.nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, edge_dim, num_layers,
        ...
        #self.out_list = []
        self.out_list = nn.ModuleList([]) #ZLJ
        #self.att_list = [] #ZLJ
        if self.brics:
            # self.out = Linear(2 * hidden_channels, out_channels)
            for _ in range(2):#LiGang
                self.out_list.append(Linear(2 * hidden_channels, out_channels))
                #self.att_list.append(Linear(2 * hidden_channels, out_channels)) #ZLJ

        else:
            # self.out = Linear(hidden_channels, out_channels)
            for _ in range(2):#LiGang
                self.out_list.append(Linear(hidden_channels, out_channels))
                #self.att_list.append(Linear(hidden_channels, out_channels)) #ZLJ
    ...
    def reset_parameters(self):
        ...
        for predictor_index in range(2):#LiGang
            self.out_list[predictor_index].reset_parameters()
            #self.att_list[predictor_index].reset_parameters() #ZLJ

        if self.cl:
            self.lin_project.reset_parameters()

    def forward(self, data, predictor_index):
        # get mol input
        x = data.x
        ...
        if self.brics:
            ...
            if self.cl:
                ...
                return self.out_list[predictor_index](out), self.lin_project(mol_vec).relu_(), self.lin_project(fra_vec).relu_()#LiGang
            else:
                out = F.dropout(out, p=self.dropout, training=self.training)
                # return self.out(out)
                return self.out_list[predictor_index](out), att#LiGang, ZLJ
                #return self.out_list[predictor_index](out)#LiGang, ZLJ

        else:
            assert self.cl is False
            out = F.dropout(mol_vec, p=self.dropout, training=self.training)
            # return self.out(out)
            return self.out_list[predictor_index](out)#LiGang
